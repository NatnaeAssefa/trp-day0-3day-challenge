...
1. **Spec-Driven Development (SDD):** \* We do not write implementation code until the Specification is ratified.  
   * We use the [**GitHub Spec Kit**](https://github.com/github/spec-kit) framework.  
...
3. **Agentic "Skills" vs. "Tools":**  
   * You must distinguish between **Skills** (reusable functions/scripts your agent calls, like download\_video) and **MCP Servers** (external bridges, like a Database connector).  
4. **Git Hygiene:**  
...
* **Task** 1.1: Deep Research **& Reading (3 Hours)**  
  ...
  * Reading List**:**  
    ...
    * [OpenClaw & The Agent Social Network](https://techcrunch.com/2026/01/30/openclaws-ai-assistants-are-now-building-their-own-social-network/)  
...
* **Task 1.2: Domain Architecture Strategy (3 Hours)**  
  ...
  * **Action:** Create a research/architecture\_strategy.md document.  
  * **Requirements:**  
...
* **Task** 2.1: The Master Specification **(4 Hours)**  
  ...
  * **Required Specs:**  
    ...
    * specs/openclaw\_integration.md: **(optional)** detailed plan on how Chimera will publish its "Availability" or "Status" to the OpenClaw network.  
...
* **Task** 2.3: Tooling & Skills **Strategy (2 Hours)**  
  ...
  * **Sub-Task A: Developer Tools (MCP):**  
    * Select and configure MCP servers that help *YOU* develop (e.g., git-mcp for version control, filesystem-mcp for file editing). Document this in research/tooling\_strategy.md.  
  * **Sub-Task B: Agent Skills (Runtime):**  
    * Define the **Skills** the *Chimera Agent* will use.  
    * *Definition:* A "Skill" is a specific capability package (e.g., skill\_download\_youtube, skill\_transcribe\_audio).  
...
* **Task 3.1: Test-Driven Development (TDD) (3 Hours)**  
  ...
  * **Deliverable:** A tests/ folder containing:  
    * test\_trend\_fetcher.py: Asserts that the trend data structure matches the API contract.  
    * test\_skills\_interface.py: Asserts that your skills/ modules accept the correct parameters.  
